...
class ResNet(nn.Module):
    def __init__(
        ...
        norm_layer=None,
        network_stride=(2, 2, 2, 2, 2),
    ):
        super(ResNet, self).__init__()
        assert len(network_stride) == 5
        ...
        self.conv1 = nn.Conv2d(
            3, self.inplanes, kernel_size=7, stride=self.network_stride[0], padding=3, bias=False
        )
        self.bn1 = norm_layer(self.inplanes)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=self.network_stride[1], padding=1)
        self.layer1 = self._make_layer(block, 64, layers[0])
        self.layer2 = self._make_layer(
            block,
            128,
            layers[1],
            stride=self.network_stride[2],
...
def resnet34(progress=True, **kwargs):
    ...
    return _resnet("resnet34", BasicBlock, [3, 4, 6, 3], progress, **kwargs)


def resnet50(progress=True, **kwargs):

    return _resnet("resnet50", Bottleneck, [3, 4, 6, 3], progress, **kwargs)


def resnet101(progress=True, **kwargs):

    return _resnet("resnet101", Bottleneck, [3, 4, 23, 3], progress, **kwargs)


def resnet152(progress=True, **kwargs):

    return _resnet("resnet152", Bottleneck, [3, 8, 36, 3], progress, **kwargs)
