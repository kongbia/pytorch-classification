...
class GhostBasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.conv1_bn_relu = GhostModule(inplanes, planes, kernel_size=3, stride=stride, relu=True)
        self.conv2_bn = GhostModule(planes, planes, kernel_size=3, relu=False)
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
...
class GhostBottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.conv1_bn_relu = GhostModule(inplanes, planes, kernel_size=1, relu=True)
        self.conv2_bn_relu = GhostModule(planes, planes, kernel_size=3, stride=stride, relu=True)
        self.conv3_bn = GhostModule(planes, planes * 4, kernel_size=1, relu=False)
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
...
class ResNet(nn.Module):
    def __init__(self, depth, num_classes=1000, block_name='BasicBlock', network_stride=(1, 1, 2, 2),):
        ...
        if block_name.lower() == 'basicblock':
            ...
            n = (depth - 2) // 9
            block = Bottleneck
        elif block_name.lower() == 'ghostbasicblock':
            assert (depth - 2) % 6 == 0, 'When use basicblock, depth should be 6n+2, e.g. 20, 32, 44, 56, 110, 1202'
            n = (depth - 2) // 6
            block = GhostBasicBlock
        elif block_name.lower() == 'ghostbottleneck':
            assert (depth - 2) % 9 == 0, 'When use bottleneck, depth should be 9n+2, e.g. 20, 29, 47, 56, 110, 1199'
            n = (depth - 2) // 9
...
if __name__ == '__main__':
    import torch
    net = resnet_cifar(block_name='BasicBlock', depth=20, num_classes=10).cuda()
